...
Reader.prototype.lastModified = function () {
  throw new Error('not implemented')
};
...
PackageReader.prototype.lastModified = function () {
  return Math.max.apply(Math, this.packages.map(function (pkg) {
    return pkg.lastModified();
  }))
};
...
Package.prototype.getFullPathRelativePathTuples = function () {
  if (this.options.main) {
    ...
    for (var i = 0; i < this.options.main.length; i++) {
      glob.sync('**/' + this.options.main[i], {cwd:this.baseDirectory}).forEach(function(val) {
        var fullPath = path.join(this.baseDirectory, val),
...
Package.prototype.lastModified = function() {
  // Race conditions, race conditions everywhere!
  return Math.max.apply(Math, this.getFullPathRelativePathTuples().map(function(tuple) {
